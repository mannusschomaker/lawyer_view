...
df_results = pd.read_csv(os.path.join(APP_PATH, os.path.join("data", "results.csv")))
df_columns = df_results.columns[6:]
params = list(df)
max_length = len(df)
    ...
    return html.Div(
        ...
        children=[
            html.Div(
                id="banner-text",
                children=[
                    html.H5("Dashboards Lawyerview"),
                    html.H6("Reporting and Analytics for Lawyerview"),
                ],
            ),
            html.Div(
                id="banner-logo",
                children=[
                    html.A(
                        html.Button(children="In Colaboration with Venturis"),
                        href="https://venturisconsulting.com/",
                    ),
                    html.Button(
                        id="learn-more-button", children="LEARN MORE", n_clicks=0
                    ),
                    html.A(
                        html.Img(id="logo", src=app.get_asset_url("itfactor.png")),
                        href="https://itfactor-consulting.nl/",
                    ),
                ],
            ),
        ],
    ...
    return html.Div(
        ...
        children=[
            html.Div(
                ...
                id="card-0",
                children=[
                    generate_section_banner("Firm Overview"),
                    # html.P("Firm selection tool"),
                    dcc.Dropdown(
                    id='firm-dropdown',
                    options=[{'label': i, 'value': i} for i in df_results['Firm'].unique()],
                    value=df_results['Firm'].unique()[0],
                    style={'width': '300px'}
                    ),
                ],
            ),
            html.Div(
                id="card-1",
                children=[
                    # html.P("Fields of expertise"),
                    dcc.Graph(id='radar-chart')  # Radar Chart placeholder
                ],
            ),
            html.Div(
                id="card-2",
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    generate_section_banner("Search Settings"),
                    dcc.RangeSlider(
                        id='number-of-lawyers-slider',
                        min=0,
                        max=100,
                        step=1,
                        value=[min(df_results["Number_of_Lawyers"]), max(df_results["Number_of_Lawyers"])],
                        marks={i: str(i) for i in range(min(df_results["Number_of_Lawyers"]), max(df_results["Number_of_Lawyers"]), 10)}
                    ),
                    # dcc.Slider(
                    #     id="num-beëdigingsdatum-young-3-slider",
                    #     min=0,
                    #     max=10,
                    #     step=1,
                    #     value=5,
                    ...
                    html.Div(
                        ...
                        children=[
                            ...
                            dl.Map(
                                ...
                                style={"height":"200px", 'border-radius': '5px','width':'100%'},
                            ),
                        ],
                        style={"margin": "5px"}
                    ),
...
# Callback to update the radar chart based on firm selection
@app.callback(
    [Output('radar-chart', 'figure'),
    Output('pie-chart', 'figure')],
    [Input('firm-dropdown', 'value')]
)
def update_radar_chart(selected_firm):
    # Filter df_results based on selected firm
    # print(selected_firm)
    firm_data_df = df_results[df_results['Firm'] == selected_firm]
    # print(firm_data_df)

        # Handle no data case here
    # print("e")
    filtered_df = firm_data_df.iloc[0]  # use the first row of df as a default
    # print(filtered_df)
    # For spider plot

    all_rechtsgebieden = df_columns
    rechtsgebieden_data = filtered_df[all_rechtsgebieden]
    ...
    rechtsgebieden_data = rechtsgebieden_data.nlargest(5)
    # print(rechtsgebieden_data)
    # print(rechtsgebieden_data.values)
    # print(rechtsgebieden_data.index)
    # print(rechtsgebieden_data.index)
    # Create radar chart using filtered data
    theta_labels = [label.replace(" ", "\n") for label in rechtsgebieden_data.index]
    # colors = sns.dark_palette("#79C", n_colors=len(2))
    # colors_hex = colors.as_hex()
    radar_chart = go.Scatterpolar(
        r=rechtsgebieden_data.values,
        theta= theta_labels,
        fill='toself',
    ...
    radar_fig = go.Figure(data=[radar_chart], 
                          layout=go.Layout(
                            ...
                            autosize=True,
                            height=300,  # Set the height to half the original size
                            width=400,
                            # template="plotly_dark",
                            polar=dict(
                                radialaxis=dict(
                                visible=True,
                                range=[0, max(rechtsgebieden_data.values)],
                                showticklabels = False
                                )),
                            paper_bgcolor="rgba(0,0,0,0)",
                            plot_bgcolor="rgba(0,0,0,0)",
    ...
    beedigings_data = firm_data_df[beedigings_cats]
    ...
    # print(beedigings_cats.index)
    labels = ['10+', '6 - 10', '3 - 6', ' 3-']
    colors = sns.dark_palette("#79C", n_colors=len(labels))
    colors_hex = colors.as_hex()
    pie_chart = go.Pie(labels=labels, values=beedigings_data.values[0], hole=0.6,marker=dict(colors=colors_hex))
    pie_fig = go.Figure(data=[pie_chart],layout=go.Layout(
                        autosize=True, height=300, width=400,
                           margin=dict(t=80, b=30, l=70, r=40),
                           plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
                           title_font=dict(size=25, color='rgba(0,0,0,0)', family="Muli, sans-serif"),
...
@app.callback(
    ...
    [Input('firm-dropdown', 'value'),
     Input('number-of-lawyers-slider', 'value')]
)
def update_bar_chart(selected_firm, num_lawyers_range):
    # Calculate Euclidean distance
    
    df_results['Distance'] = df_results[df_columns].apply(lambda row: distance.euclidean(row, df_results[df_results['Firm'] == selected_firm].iloc[0][df_columns].values), axis=1)

    # Normalize distances to obtain similarity scores
    max_distance = df_results['Distance'].max()
    min_distance = df_results['Distance'].min()
    df_results['Similarity'] = 100 - ((df_results['Distance'] - min_distance) / (max_distance - min_distance)) * 100

    # Filter based on number of lawyers range
    filtered_results = df_results[
        (df_results['Number_of_Lawyers'] >= num_lawyers_range[0]) &
        (df_results['Number_of_Lawyers'] <= num_lawyers_range[1])
    ]

    # Get top 10 highest scoring firms
    top_10_firms = filtered_results.nlargest(10, 'Similarity')
    print(top_10_firms.head())
    # Create a bar chart
    fig = go.Figure(data=go.Bar(
        x=top_10_firms['Firm'],
        y=top_10_firms['Similarity'],
        marker_color='steelblue',
    ))

    fig.update_layout(
        # title='Top 10 Similar Firms',
        # xaxis_title='Firm',
        yaxis_title='Match %',
        template='plotly_dark',
        plot_bgcolor='rgba(0, 0, 0, 0)',
...
def handle_bar_chart_click(click_data):
    if click_data is not None:
        selected_firm = click_data['points'][0]['x']
        # Filter the data based on the selected firm
        selected_data = df_results[df_results['Firm'] == selected_firm]

        # Create a DataTable to display the selected data
    ...
    # Display a text message when no bar is clicked
    return html.P("Select a firm")
